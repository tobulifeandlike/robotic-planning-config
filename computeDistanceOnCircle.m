function d = computeDistanceOnCircle(theta1, theta2)
   if mod(theta2,2*pi)>=mod(theta1,2*pi)%initially we are not sure which angle is larger, so we need to set a comparison condition to confirm which angle is smaller
